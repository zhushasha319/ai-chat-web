...
:deep(.composer-textarea .el-textarea__inner) {
  ...
  border: none;
  box-shadow: none;
  /* 增加底部 padding 防止出现内容高度与计算行高 1~2px 的差值而导致早早出现滚动条 */
  padding: 6px 8px 6px 8px;
  resize: none; /* 禁止右下角拖拽 */
  font-size: 14px;
  line-height: 1.6;
  color: var(--el-text-color-primary);
  /* 让 autosize 情况下单行时不出现滚动条，超过 maxRows(8) 后仍可滚动 */
  overflow-y: auto;
}
/* 隐藏在内容不足以滚动时的滚动条视觉（WebKit） */
:deep(.composer-textarea .el-textarea__inner::-webkit-scrollbar) {
  width: 8px; /* 需要完全隐藏可改为 0 */
}
:deep(.composer-textarea .el-textarea__inner:disabled) {
  cursor: not-allowed;
}
