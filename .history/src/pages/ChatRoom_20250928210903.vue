      ...
      <div class="sidebar-header">
        ...
        <!-- <div class="api-mode-switch">
          <el-switch
            v-model="useMockAPI"
            active-text="Mock"
            inactive-text="Real API"
            active-color="#13ce66"
            inactive-color="#ff4949"
            size="small"
          />
          <div class="mode-hint">
            {{ useMockAPI ? "🔧 测试模式" : "🚀 真实 API" }}
          </div>
        </div> -->
      </div>
...
const { scrollRef, scrollToBottom, maybeAutoScroll, onUserScroll, autoStick } =
  useAutoScroll();
...
// API 模式切换
const useMockAPI = ref(false); // 默认使用真实 API（千问）
...
const title = computed({
  get: () => current.value?.title || "",
  set: (v) => current.value && (current.value.title = v),
});
function saveTitle() {
  current.value && conversations.renameChat(current.value.id, title.value);
}
// 双向绑定当前对话标题，失焦时保存
...
async function onSend(text: string) {
  ...
  try {
    const apiUrl = useMockAPI.value
      ? '/api/debug/stream'  // 测试模式使用调试端点
      : '/api/chat/stream';  // 生产模式使用聊天端点
    const model = "qwen-turbo"  // 使用正确的模型名称
    await start(apiUrl, { model, messages }, (chunk) => {
      ...
      if (currentMsg) {
        ...
        console.warn("[Chat] 未找到目标消息 aid=", aid);
      }
      maybeAutoScroll();
    });
  } catch (e: any) {
