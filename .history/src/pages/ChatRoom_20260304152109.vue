...
async function onSend(text: string) {
  ...
  try {
    ...
    await start(
      ...
      },
      {
        ...
        onError: (err) => {
          ...
          const msgRef = current.value?.messages.find((m: Message) => m.id === aid);
          if (msgRef) {
            msgRef.content = (msgRef.content || "") + `\n\n[错误] ${err.message}`;
            msgRef.meta = { ...(msgRef.meta || {}), streaming: false, error: err.message };
          }
    ...
    const currentMsg = current.value!.messages.find((m: Message) => m.id === aid);
