...
// VirtualChatList 组件引用
const chatListRef = ref<InstanceType<typeof VirtualChatList> | null>(null);
...
const confirmDelete = (id: string, title: string) => {
  ElMessageBox.confirm(
    ...
    .then(() => {
      // Remove the conversation from items array
      const index = conversations.items.findIndex((c) => c.id === id);
      if (index !== -1) {
        conversations.items.splice(index, 1);
        // If deleted conversation was current, switch to first available
        if (conversations.currentId === id && conversations.items.length > 0) {
          const firstItem = conversations.items[0];
          if (firstItem) {
            conversations.currentId = firstItem.id;
            router.push(`/chat/${firstItem.id}`);
          }
        }
      }
    })
...
async function onSend(text: string) {
  ...
  // 发送消息后，等待 DOM 更新，然后平滑滚动到底部（仿 ChatGPT）
  await nextTick();
  autoStick.value = true; // 重置自动吸底状态
