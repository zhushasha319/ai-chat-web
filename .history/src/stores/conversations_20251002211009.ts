...
export const useConversations = defineStore("conversations", {
  ...
  actions: {
    ...
    _ensureRuntimeMaps() {
      if (!(this._convMap instanceof Map))
        this._convMap = new Map<string, Conversation>();
      if (!(this._msgMap instanceof Map))
        this._msgMap = new Map<string, Map<string, Message>>();
      if (!(this._streamBuffers instanceof Map))
        this._streamBuffers = new Map<
          string,
          { pending: string; raf: number | null }
        >();
    },
